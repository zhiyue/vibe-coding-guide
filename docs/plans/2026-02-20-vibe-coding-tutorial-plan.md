...
6. **认识终端/命令行**（400-500 字）
   ...
   - 试一试：输入 `echo "Hello"` 看看效果（截图）
   - 说明：后面章节中，偶尔会用到终端来安装工具或运行命令，遇到时会手把手教
...
7. **按路线准备环境**（500-700 字）
   ...
   - **路线 B：编辑器端工具（Cursor）**
     - Mac 用户需要安装 Homebrew（包管理器，类比为"软件的应用商店"）：
       - 打开终端，粘贴安装命令（给出具体命令）
       - 等待安装完成（可能需要几分钟）
       - 验证：输入 `brew --version`，看到版本号就成功了（截图）
...
4. **路线 B：Cursor 上手指南**（1200-1500 字）
   - **环境检查**（先确认第 2 章的准备工作完成了）：
     - 检查 Node.js 是否已安装：`node --version`
     - 如果没有，回到第 2 章"路线 B"按步骤安装
   - 下载安装 Cursor（截图）
...
6. **常见问题 FAQ**（600-800 字）
   ...
   - Node.js 安装后 `node` 命令找不到？（PATH 问题简单解释和解决方法）
   - 安装时提示需要管理员权限怎么办？
