...
1. 在浏览器中打开 https://lovable.dev
2. 点击页面上的 "Sign Up" 或 "Get Started"
3. 你可以用 Google 账号、GitHub 账号或者邮箱注册
4. 注册完成后，你就能看到 Lovable 的主界面了

<!-- screenshot: Lovable 首页，标注注册/登录按钮的位置 -->
...
<!-- screenshot: Lovable 主界面标注三个区域：对话、预览、文件 -->
...
1. 先试试**关掉终端再重新打开**
2. 如果还不行，**重启电脑**
3. 如果还不行，重新运行安装程序，找到 "Add to PATH" 的选项确保勾选

**手动添加 PATH（进阶）：** 如果上面的方法都不行，你可以手动添加：
1. 按 `Win + S` 搜索"环境变量"，点击"编辑系统环境变量"
2. 点击"环境变量"按钮
3. 在"用户变量"或"系统变量"中找到 `Path`，双击编辑
4. 添加对应工具的安装路径（比如 Node.js 默认在 `C:\Program Files\nodejs\`）
5. 确定保存，重新打开终端

> 如果你觉得手动配 PATH 太复杂，最简单的办法就是**卸载后重新安装，安装时注意勾选所有 PATH 相关选项**。
